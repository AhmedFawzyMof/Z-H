...
Products.forEach(function (product) {
  ...
  if (res !== undefined) {
    ...
    QuantityIncBtn.setAttribute(
      "onclick",
      "incQuantity(".concat(res.id, ", ").concat(res.quantity + 1, ")")
    );
...
function addItemToCart(productId) {
  ...
  var stock = Product.querySelector("#inStock").value;
  var quantity = 1;
  var product = {
    id: id,
    name: name,
...
function incQuantity(productId, quantity) {
  ...
  plusOne.outerHTML = '<button id="Inc" onclick="incQuantity('
    .concat(res.id, ", ")
    .concat(res.quantity + 1, ')"><i class="bx bx-plus"></i></button>');
}
